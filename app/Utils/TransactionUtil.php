...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'paid_money' => !empty($input['paid_money']) ? $input['paid_money'] : null,
            'change_money_data' => !empty($input['change_money_data']) ? $input['change_money_data'] : null,
            'comments' => !empty($input['comments']) ? $input['comments'] : null,
            'address_id' => !empty($input['address_delivery']) ? $input['address_delivery'] : null,
        ]);
    ...
    {
        $il = $invoice_layout;
        $this->checkLanguageForReceipt($il);
        ...
        //Logo
        $output['logo'] = $il->show_logo != 0 && !empty($il->logo) && file_exists(base_path('uploads/img/' . $il->logo)) ? asset('uploads/img/' . $il->logo) : false;
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_mobile'] = $customer && $customer->mobile ? $customer->mobile : '';

            if (!empty($output['customer_name']) && $receipt_printer_type != 'printer') {
                $output['customer_info'] .= $customer->contact_address ? $customer->contact_address : '';
                if (!empty($customer->contact_address)) {
                    $output['customer_info'] .= '<br>';
                }
                $output['customer_info'] .= $customer && $customer->mobile ? $customer->mobile : '';
                if (!empty($customer->landline)) {
                    $output['customer_info'] .= ', ' . ($customer && $customer->landline ? $customer->landline : '');
                }
            }

            $output['customer_tax_number'] = $customer && $customer->tax_number ? $customer->tax_number : '';
    ...
    public function checkLanguageForReceipt($invoice){

            $invoice->invoice_no_prefix = trans('lang_v1.invoice_no');
            $invoice->date_label        = trans('lang_v1.date');
            $invoice->table_product_label        = trans('lang_v1.table_product_label');
            $invoice->table_qty_label        = trans('lang_v1.table_qty_label');
            $invoice->table_unit_price_label        = trans('lang_v1.table_unit_price_label');
            $invoice->table_subtotal_label        = trans('lang_v1.table_subtotal_label');
            $invoice->sub_total_label        = trans('lang_v1.invoice_subtotal_label');
            $invoice->line_discount_label        = trans('lang_v1.line_discount_label');
            $invoice->paid_label        = trans('lang_v1.total_paid_label');
            $invoice->total_label        = trans('lang_v1.total_label');
            $invoice->discount_label        = trans('lang_v1.discount_label');
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        $data['net_profit'] = $module_total + $gross_profit
            + ($data['total_sell_round_off'] + $data['total_recovered'] + $data['total_sell_shipping_charge'] + $data['total_purchase_discount']) - ($data['total_reward_amount'] + $data['total_expense'] + $data['total_adjustment'] + $data['total_transfer_shipping_charges'] + $data['total_purchase_shipping_charge']);
    ...
    public function payContact($request, $format_data = true)
    {
      ...
      /**
     * Add Sell transaction From Ecommerce
     *
     * @param int $business_id
     * @param array $input
     * @param float $invoice_total
     ...
     * @return boolean
     */
    public function createSellTransactionEcommerce($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
        $invoice_no = !empty($input['invoice_no']) ? $input['invoice_no'] : $this->getInvoiceNumber($business_id, $input['status'], $input['location_id'], $invoice_scheme_id);
        ...
        $transaction = Transaction::create([
            ...
            'invoice_no' => $invoice_no,
            'ref_no' => '',
            'total_before_tax' => $invoice_total,
            'transaction_date' => $input['transaction_date'],
            'tax_id' =>  null,
            'discount_type' => !empty($input['discount_type']) ? $input['discount_type'] : null,
            'discount_amount' => $uf_data ? $this->num_uf($input['discount_amount']) : $input['discount_amount'],
     ...
     * @return boolean/object
     */
    public function createOrUpdateSellLinesEcommerce($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $uf_item_tax = $uf_data ? $this->num_uf($product['item_tax']) : $product['item_tax'];
                $uf_unit_price_inc_tax = $uf_data ? $this->num_uf($product['unit_price_inc_tax']) : $product['unit_price_inc_tax'];
                $variation_id=Variation::where('product_id',$product['product_id'])->first();
                $line = [
                    'product_id' => $product['product_id'],
